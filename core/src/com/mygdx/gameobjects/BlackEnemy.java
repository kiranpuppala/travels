...
public class BlackEnemy {
   ...
   public static Rectangle dummyForBlack1=new Rectangle();
    ...
    public void update(float delta) {
        blackEnemyRect.set((float) x, (float) y, AssetLoader.blackenemy.getWidth(), AssetLoader.blackenemy.getHeight());

        if(CollDet.collided(blackEnemyRect,BlackEnemy.dummyForBlack1,0,0)==CollDet.LEFT_SIDE){
            vx=0;
            moveRight=true;
            moveLeft=false;
        }

        if(CollDet.collided(blackEnemyRect, BlackEnemy.dummyForBlack2,0,0)==CollDet.RIGHT_SIDE){
            vx=0;
            moveRight=false;
            moveLeft=true;
        }
